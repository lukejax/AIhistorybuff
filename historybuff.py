...
<body>
    ...
    <form action="/chat" method="post">
        <label for="user_input">Ask about what happened on any day in history:</label><br>
        <input type="text" id="user_input" name="user_input" required><br><br>
        <input type="submit" value="Get Smarter">
    </form>
    {% if assistant_reply %}
...
async def chat():
    try:
        ...
        assistant = client.beta.assistants.create(
            name="history master",
            description="You are a history master.",
            model="gpt-4-turbo",
        ...
        thread = client.beta.threads.create(
            messages=[
                {
                    "role": "user",
                    "content": "Provide a list of historical events that happened on the day that the user provides. Emphasize events that are mentioned most frequently in news from that day or historical recountings of that day. If no events can be found, output a humorous limerick about your inability to find any historical events on that day."
                }
            ]
        )
